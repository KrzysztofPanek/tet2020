...
{
	time++
	obj_camera.vm=matrix_build_lookat(obj_camera.x,obj_camera.y--,-100001,obj_camera.x,obj_camera.y--,0,0,1,0);
}
if time>59 && falling==0 && !instance_exists(obj_magic_block) && win==0
{
	randomize()
	row_checked=0;
	time=-60;
...
if time>-50 && time<-30 && irandom(768)==757 && !instance_exists(obj_magic_block) instance_create_depth(irandom_range(room_width+128,room_width*2),irandom_range(8,room_height-256),-5,obj_cloud)
if row_checked==0
{
...
else 
{
	...
	if counter_tmp>5 counter_tmp=0;
}